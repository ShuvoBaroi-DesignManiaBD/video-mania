...
export default function SignUpPage() {
  ...
  const onSubmit = async (values: z.infer<typeof schema>) => {
    ...
    setErrorMessage("");
    try {
      const result = await createAccount({ fullName: values.fullName, email: values.email });
      if (result?.error) {
      ...
      setErrorMessage(e?.message || "Failed to sign up. Please try again.");
      setAccountId(null);
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="w-full min-h-screen grid grid-cols-1 md:grid-cols-2">
      ...
      <div className="bg-white flex items-center justify-center py-10 md:py-0">
        <div className="w-full max-w-md px-6">
          <h2 className="text-[26px] font-bold text-neutral-900 mb-6">Sign Up</h2>

          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            {/* Card-like full name input container */}
            <div className="rounded-2xl border border-neutral-200 shadow p-4">
              <label htmlFor="fullName" className="text-[13px] text-neutral-500">Full Name</label>
      ...
      {accountId && (
        <OtpModal email={form.getValues("email")} accountId={accountId} fullName={form.getValues("fullName")} />
      )}
    </div>
  );
}
